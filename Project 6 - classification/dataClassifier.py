...
def enhancedFeatureExtractorDigit(datum):
    ...
    features["empty%"] = float(len(emptyPixels))/(DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT)

    while len(emptyPixels):
        regions+=1
        pixelStack.push(emptyPixels.pop())
        while not pixelStack.isEmpty():
            x,y = pixelStack.pop()
            for pixelN in neighbours(x,y):
                if pixelN in emptyPixels:
                    emptyPixels.remove(pixelN)
                    pixelStack.push(pixelN)

    for i in range(1,4):
        if i == regions:
            features["free" + str(i)] = 1
        else:
            features['free' + str(i)] = 0


    # %lini zapelnionych bardziej niz polowa
    pionowych = 0
    for x in range(DIGIT_DATUM_WIDTH):
        pelna = -DIGIT_DATUM_HEIGHT/2
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x,y)!=0: pelna+=1
        if pelna>=0: pionowych+=1
   
    poziomych = 0
    for y in range(DIGIT_DATUM_HEIGHT):
        pelna = -DIGIT_DATUM_WIDTH/2
        for x in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y)!=0: pelna+=1
        if pelna>=0: poziomych+=1
    
    features['poziomych'] = poziomych/float(DIGIT_DATUM_HEIGHT)
    features['pionowych'] = pionowych/float(DIGIT_DATUM_WIDTH)
    ...
    return features
def dealWithPixel(x,y,freeRegions,datum):
    if datum.getPixel(x,y)>0:
        return

    for xyN in neighbours(x,y):
        for freeRegion in freeRegions:
            if xyN in freeRegion:
               freeRegion.add((x,y))
               return
    newFreeRegion = set()
    newFreeRegion.add((x,y))
    freeRegions.append(newFreeRegion)
    return


def neighbours(x, y):
    n = list()
    if x+1<DIGIT_DATUM_WIDTH: n.append((x+1,y))
    if x-1>=0: n.append((x-1,y))
    if y+1<DIGIT_DATUM_HEIGHT: n.append((x,y+1))
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostStates = newState.getGhostStates()
    foodPos = newState.getFood().asList()
    normalGhostsPoss = [g.getPosition() for g in ghostStates if g.scaredTimer==0]
    scaredGhostPoss = [g.getPosition() for g in ghostStates if g.scaredTimer!=0]

    if len(normalGhostsPoss):
        minGhostDistance = min(util.manhattanDistance(gPos,pacmanPos) for gPos in normalGhostsPoss)
        if minGhostDistance < 2:
            features["close to ghost"] = 1

    features["min ghost good"] = 0.1
    if len(normalGhostsPoss):
        minGhostDistance = min(util.manhattanDistance(gPos,pacmanPos) for gPos in normalGhostsPoss)
        features["min ghost good"]  += 0.01*minGhostDistance

    if len(scaredGhostPoss):
        minGhostGood = min(util.manhattanDistance(gPos,pacmanPos) for gPos in scaredGhostPoss)
        if minGhostGood < 2:
            features["close to ghost good"] = 1

    if len(foodPos):
        features["min food"] = min(util.manhattanDistance(pacmanPos,f) for f in foodPos)
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    #         print "Image: "
    #         print rawTestData[i]
    #         print testData[i]["free1"]
    #         print testData[i]["free2"]
    #         print testData[i]["free3"]
    #         print testData[i]["poziomych"]
    #         print testData[i]["pionowych"]
    #         break
