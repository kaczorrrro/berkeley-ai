...
class BayesNet(object):
    ...
    def variablesSet(self):
        " Retuns a copy of the set of variables in the bayes net "
        return copy(self.__variablesSet)

    def variableDomainsDict(self):
        " Retuns a copy of the variable domains in the bayes net "
        return deepcopy(self.__variableDomainsDict)

    def inEdges(self):
        " Retuns a copy of the incoming edges in the bayes net "
        return deepcopy(self.__inEdges)

    def outEdges(self):
        " Retuns a copy of the outgoing edges in the bayes net "
    ...
    def __str__(self):
        ...
        return netString + factorsString
    ...
    def easierToParseString(self):
        " Used internally for computer-readable printing "
        returnStrings = []
        for CPT in self.getAllCPTsWithEvidence():
            prefix = CPT.unconditionedVariables()[0]
            returnStrings.append(CPT.easierToParseString(prefix=prefix, printVariableDomainsDict=False))
        return "\n".join(returnStrings)
...
class Factor(object):
    ...
    def __init__(self, inputUnconditionedVariables, inputConditionedVariables, inputVariableDomainsDict):
        ...
        self.__unconditionedVariables = inputUnconditionedVariables
        self.__conditionedVariables = inputConditionedVariables
