...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        #print "query:" + str(queryVariables) + " given " + str(evidenceDict.keys())
        ...
        factorsToDealWith = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for varToEliminate in eliminationOrder:      
            factorsToDealWith, joinEffect = joinFactorsByVariable(factorsToDealWith,varToEliminate)
            #if factor has only one unconditioned variable it would be eliminated so this factor is just dropped
            if len(joinEffect.unconditionedVariables()) == 1:
                continue

            eliminateEffect = eliminate(joinEffect,varToEliminate)
            factorsToDealWith.append(eliminateEffect)
            # print "polaczone!"
            # print eliminateEffect
        # print "final"
        if len(factorsToDealWith) == 1:
            return factorsToDealWith[0]
        else:
            joined = joinFactors(factorsToDealWith)
...
def inferenceByLikelihoodWeightingSamplingRandomSource(randomSource=None):
    ...
    def inferenceByLikelihoodWeightingSampling(bayesNet, queryVariables, evidenceDict, numSamples):
        ...
        factor = Factor(queryVariables,evidenceDict.keys(),bayesNet.getReducedVariableDomains(evidenceDict))
        evidence = evidenceDict.keys()
        ...
        for x in range(0,numSamples):
            ...
            for var in bayesNet.linearizeVariables(): 
                if var in evidence:
                    asignmentD[var] = evidenceDict[var]
                    #print bayesNet.getCPTWithEvidence(var,asignmentD).getProbability(asignmentD)
                    weight *= bayesNet.getCPTWithEvidence(var,asignmentD).getProbability(asignmentD)
                    
                else:
                    tempFactor = bayesNet.getCPTWithEvidence(var,asignmentD)
                    asignmentD[var] =  sampleFromFactor(tempFactor,asignmentD)[var]
                checkedVars.add(var)



            factor.setProbability(asignmentD,factor.getProbability(asignmentD) + weight)
