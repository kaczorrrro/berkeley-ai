...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance==None:
            ...
            for position in self.legalPositions:
                trueDistance = manhattanDistance(pacmanPosition,position)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in oldBeliefs:
            oldProb = oldBeliefs[oldPos]
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,oldPos))
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particlesList = []
        particlesAdded = 0
        maxParticles = self.numParticles
        while True:
            for pos in self.legalPositions:
                self.particlesList.append(pos)
                particlesAdded +=1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for x in range(0,self.numParticles):
                newParticlesList.append(jail)
            self.particlesList = newParticlesList
            return

        for particle in self.particlesList:
            dist[particle] += emissionModel[util.manhattanDistance(pacmanPosition,particle)]
        ...
        for x in range(0,self.numParticles):
            newParticlesList.append(util.sample(dist))
        self.particlesList = newParticlesList
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particlesList:
            ...
            newParticlesList.append(util.sample(newPosDist))
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        currentDist = util.Counter()

        for particle in self.particlesList:
            currentDist[particle] +=1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        allPossible = []
        import itertools
        for posProd in itertools.product(self.legalPositions, repeat=self.numGhosts):
                allPossible.append(posProd)

        from random import shuffle
        shuffle(allPossible)     
        listLen = len(allPossible)   
        for x in range(0,self.numParticles):
            self.particlesList.append(allPossible[x%listLen])
    ...
    def observeState(self, gameState):
        ...
        self.ghostsToJailParticles(noisyDistances)

        newJointDist = util.Counter()

        for particle in self.particlesList:
            weight = 1
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] != None:
                    weight *= emissionModels[ghost][util.manhattanDistance(particle[ghost],pacmanPosition)]
            newJointDist[particle] += weight

        if newJointDist.totalCount() == 0:
            self.initializeParticles()
            self.ghostsToJailParticles(noisyDistances)
            return

        newJointDist.normalize()
        newParticlesList = []
        for x in range(self.numParticles):
            newParticlesList.append(util.sample(newJointDist))
        self.particlesList = newParticlesList

    def ghostsToJailParticles(self, noisyDistances):
        updatedParticlesList = []
        for x in range(self.numGhosts):
            if noisyDistances[x] == None:
                updatedParticlesList = []
                for particle in self.particlesList:
                    updatedParticlesList.append(self.getParticleWithGhostInJail(particle,x))
                self.particlesList = updatedParticlesList
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.particlesList:
            newParticle = list(oldParticle) # A list of ghost positions
            # now loop through and update each entry in newParticle...
            "*** YOUR CODE HERE ***"
            for ghost in range(self.numGhosts):
                newDistribution = getPositionDistributionForGhost(setGhostPositions(gameState,oldParticle),ghost,self.ghostAgents[ghost])
                newParticle[ghost] = util.sample(newDistribution)
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        jointProb = util.Counter()
        for possiblePos in self.particlesList:
            jointProb[possiblePos] +=1
        jointProb.normalize()
        return jointProb
